...
module.exports = class extends Base {
  async indexAction() {
    ...
    if (data.totaloxdiant == undefined || data.totaloxdiant=='' ) { data.totaloxdiant=0 };
    if (data.nowoxdiant == undefined || data.nowoxdiant=='' ) { data.nowoxdiant=0 };
    if (data.totalfuel == undefined || data.totalfuel=='' ) { data.totalfuel=0 };
    if (data.nowfuel == undefined || data.nowfuel=='' ) { data.nowfuel=0 };
    ...
    if(data.powerstyle!=='单组元' && data.powerstyle!=='双组元' && data.powerstyle!=='双组元+电推'){
    	...
    	if (data.FUELtotal == undefined) { data.FUELtotal=0 };
    	if (data.FUELnow == undefined) { data.FUELnow=0 };
    	if (data.FUELpercent == undefined ) { data.FUELpercent= 100 };
  	}
    ...
    if (data.orbit_remarks == undefined) { data.orbit_remarks='' };
   ...
   let trbs = [];
    ...
    if (data.life == undefined || data.life=='' ) { data.life='-' };
    if (data.assessLife == undefined || data.assessLife=='' ) { data.assessLife='-' };
    data.outTime = data.retireTime
    if (data.outTime == undefined || data.outTime=='' ) { data.outTime=data.invalidTime };
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      if (typeof(codes) === 'string' || (codes!=null && codes.indexOf(code) > -1)) {
        const testreport = this.mongo('testreport', {database: groups[i].code});
        const grpcount = await testreport.where({state: '已发布'}).count();
        reportcount+=grpcount;

        const stage = this.mongo('stage', {database: groups[i].code});
        const stacount = await stage.where().count();
        stagecount+=stacount;
      }
    }
    const repor = this.mongo('testreport', {database: code});
    const satcount = await repor.where({state: '已发布'}).count();
    reportcount+=satcount;
    this.assign('reportcount', reportcount);

    const stag = this.mongo('stage', {database: code});
    const stages = await stag.where().count();
    stagecount+=stages;
    this.assign('stagecount', stagecount);
  ...
  async singleReportAction() {
    ...
    const testreport = this.mongo ('testreport', {database: code});
    let data = await testreport.where({'state':'已发布'}).select();
  ...
  async getZhibiaoAction(){
    ...
    //查询指标表获取待展示的指标
    const analysisdb = this.mongo('analysis', {database:code});
	let analysiscollection=null;
	if(subsysid!=null && subsysid!='' && subsysid!=undefined){
		analysiscollection = await analysisdb.where({subsysfocus: 'true',subsys:subsysid}).select();//使用型号展示的要求查询
	}else{
		if(deviceid!=null && deviceid!='' && deviceid!=undefined){
			analysiscollection = await analysisdb.where({devicefocus: 'true',device:deviceid}).select();//使用型号展示的要求查询
		}else{
			analysiscollection = await analysisdb.where({satfocus: 'true'}).select();//使用型号展示的要求查询
		}

	}
    const materialdb = this.mongo('material', {database:code});
    const result = [];
    for(let i=0;i<analysiscollection.length;i++){
      ...
      for(let j=0;j<mats.length;j++){
        let matitem={};
        matitem.end=mats[j].end.substr(0,10);
        matitem.begin=mats[j].begin.substr(0,10);
		if(analysiscollection[i].resource_type=='table'){
  ...
  async getstageAction(){
    ...
    let sat =await satellite.where({_id:_satid}).find();
	  ...
	  for(let i=0;i<data.length;i++){
	  	let xiangmu = await result.where({'stage_name':data[i].name}).select();
	  	let arr = [];
	  	let newArr = [];
	  	let num;
	  	if(xiangmu != ""){
	  		for(let j=0;j<xiangmu.length;j++){
	  			arr.push(xiangmu[j].project);
	  		}
	  		for(var z = 0; z < arr.length;z++){
          if(newArr.indexOf(arr[z])== -1){
              newArr.push(arr[z]);
          }
      	}
	  		num=newArr.length;
