...
module.exports = class extends Base {
  async maintainAction() {
    ...
    this.assign('sat', sat);
    const doc_tar = this.mongo('basic_regulate', {database: sat.code});
  ...
  async addAction() {
    ...
    this.assign('sat', sat);
    return this.display('home/regulate_edit.html');
  }
  ...
  async getListAction() {
    ...
    const satellite = this.mongo('satellite');
    let sat = await satellite.where({_id: _satid}).find();
    const doc_channel = this.mongo('basic_regulate', {database: sat.code});
  ...
  async doSaveAction() {
    ...
    if (_id == undefined || _id == null || _id == '') {
      let sameregulate = await doc_tar.where({$and: [{code: data.code}, {$or: [{valid: {$exists: false}}, {valid: true}]}]}).select();
      if (sameregulate && sameregulate.size > 1) {
        return this.json({success: false, msg: '数据类型代号已被占用', data: sameregulate});
      }
  ...
  async doValidateCodeAction(){
    const _satid = this.post('_satid');
    const tarid = this.post('_regulateid');
